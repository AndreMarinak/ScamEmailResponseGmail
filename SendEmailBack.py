...
from config import GMAIL_SCOPES, CREDENTIALS_FILE, TOKEN_FILE, check_credentials
...
def send_analysis_response(service, sender, subject, content):
    """Send an analysis response back to the sender."""
    response_body = f"""
    Hello,

    Here's an analysis of the email you sent:
    ...
    message['to'] = sender
    message['subject'] = f"Analysis of Your Email: {subject}"
...
def main():
    ...
    if not check_credentials():
        print("Missing credentials. Please set up your credentials first.")
        return
    ...
    for email_meta in unread_emails:
        ...
        # Analyze the email
        sender, subject, content = analyze_email(message)
        print(f"Sender: {sender}")
        print(f"Subject: {subject}")
        #print(f"Contents: {content}")

        # Send the analysis response back to the sender
        send_analysis_response(service, sender, subject, content)
